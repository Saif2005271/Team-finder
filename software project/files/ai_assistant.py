"""
Team-Finder — AI Assistant Integration
========================================
Wraps the Anthropic Claude API to power the in-platform AI assistant.
The assistant is context-aware: it receives the current user's profile
so it can give personalised, actionable advice rather than generic answers.

Key design decisions to minimise token usage (and therefore API cost):
  - The system prompt is compact and role-specific.
  - Only the last 6 conversation turns are forwarded (sliding window).
  - Profile data is summarised, not serialised in full.
"""
